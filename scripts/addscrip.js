$(document).ready(function() {
  ...
  modalForm +=
    '<span aria-hidden="true">&times;</span></button></div><div class="modal-body"><div class="input-group mb-3">';
  modalForm +=
    '<div class="input-group-prepend"><label class="input-group-text" for="inputGroupSelect01">Options</label>';
  modalForm +=
    '</div></div><div class="input-group mb-3"><div class="input-group-prepend">';
  modalForm +=
    '<span class="input-group-text" id="inputGroup-sizing-default">Quantity</span>';
  modalForm +=
    '</div><input type="number" class="form-control" aria-label="Sizing example input" size="4" aria-describedby="inputGroup-sizing-default"></div>';
  modalForm +=
    '</div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>';
  modalForm +=
    '<button type="button" class="btn btn-primary">Buy</button></div></div></div></div>';
  ...
  $.ajax({
    ...
    success: function(stock_symbol) {
      ...
      for (var key in stock_symbol) {
        scrip_list +=
          "<option value=" + key + ">, " + stock_symbol[key] + "</option>";
      }
      scrip_list += "</select>";
