...
function liveFeed() {
  ...
  var i, arg;
  $.ajax({
    //start ajax1 for live 3 stock prices at random every minute() - LIVE STOCK UPDATES
...
>>>>>>> 58566482ba3769ccd0e7a69e37168d95635b1bcb
                }
                j++;
              }
            }
<<<<<<< HEAD
  ...
  $.ajax({
    //start ajax3  : for LIVE INDIAN MARKET INDICES
      ...
      for (var p = 0; p < mkt_indx_data.length; p++) {
        ...
        $.ajax({
          ...
          success: function(mktIndxOut) {
            $("#MarketIndex li.list-group-item")
              .eq(q)
              .text(
                mktIndxOut.data[0]["name"] +
                  "       " +
                  mktIndxOut.data[0]["price"]
              );
            q++;
            // }
          }
  ...
  $.ajax({
    //start ajax5  : for LIVE GLOBAL MARKET INDICES
      ...
      for (var n = 0; n < global_indx_data.length; n++) {
        ...
        $.ajax({
          ...
          success: function(globalIndxOut) {
            $("#GlobalIndex li.list-group-item")
              .eq(m)
              .text(
                globalIndxOut.data[0]["name"] +
                  "      -       " +
                  globalIndxOut.data[0]["price"]
              );
            m++;
            // }
          }
